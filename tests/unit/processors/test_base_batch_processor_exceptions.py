...
def test_hook_exception_logged(tmp_path):
    ...
    found = any("Hook failure" in str(call.args) for call in processor.logger.exception.call_args_list)
...
def test_cleanup_exception_logged(tmp_path):
    ...
    assert processor.logger.exception.called
    assert any("Cleanup failed" in str(call.args) for call in processor.logger.exception.call_args_list)

...
def test_setup_loads_data(processor):
    with patch("portrait_quality_batch_processor.os.path.exists", return_value=False), \
         patch("builtins.open", create=True), \
         patch.object(processor, "load_image_data", return_value=[{"file_name": "img1.jpg", "orientation": "1", "bit_depth": "8"}]):

        processor.setup()
        assert processor.data == [{"file_name": "img1.jpg", "orientation": "1", "bit_depth": "8"}]
...
def test_process_batch_skips_all(processor):
    ...
    processor.logger.info.assert_any_call("All images in this batch are already processed. Skipping.")

def test_process_batch_processes_one(processor, tmp_path):
    processor.setup()
    processor.result_csv_file = tmp_path / "output" / f"evaluation_results_{processor.date}.csv"
...
def test_process_single_image_marks_and_returns_result(processor):
    processor.process_image = MagicMock(
        side_effect=lambda path, orientation, bit_depth: {"file_name": "img1.jpg", "sharpness_score": 0.9}
    )
    processor._mark_as_processed = MagicMock()

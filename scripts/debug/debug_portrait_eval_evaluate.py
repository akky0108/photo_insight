...
def main() -> int:
    if len(sys.argv) < 2:
        print(
            "usage: python scripts/debug/debug_portrait_eval_evaluate.py <nef_path>",
            flush=True,
        )
        return 2

    nef_path = Path(sys.argv[1]).expanduser().resolve()
    print(f"[START] evaluator evaluate test: {nef_path}", flush=True)
    ...
    print("[7] before evaluator.evaluate()", flush=True)
    result = evaluator.evaluate()
    print("[8] after evaluator.evaluate()", flush=True)

    print(f"[RESULT_TYPE] {type(result).__name__}", flush=True)
    if isinstance(result, dict):
        print(f"[RESULT_KEYS] {sorted(result.keys())}", flush=True)

    print("[END] evaluator evaluate test ok", flush=True)
    return 0

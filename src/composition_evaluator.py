    ...
    def evaluate_batch(self, image_paths):
        ...
        for image_path in image_paths:
            try:
                ...
                self.logger.error(f"Error evaluating {image_path}: {e}")
...
if __name__ == "__main__":
    ...
    for image_path, score in batch_scores.items():
        if score is not None:
            print(f"{image_path} - Composition Score: {score:.2f}")
        else:
            print(f"{image_path} - Evaluation Failed")

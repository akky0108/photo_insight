...
import pandas as pd
import shutil
from batch_framework.base_batch import BaseBatchProcessor  # バッチ処理の基底クラスをインポート
...
class CsvToTxtProcessor(BaseBatchProcessor):
    ...
    def setup(self):
        ...
        self.output_dir = os.path.join(self.project_root, "processed_txt")  # 出力ディレクトリ
    ...
    def process(self):
        ...
        try:
            ...
            self.logger.info(f"Step 1: face_sharpness_evaluation >= 4 records: {len(df_face_sharpness)}")
            
            # ステップ2: ①で除外されたデータから overall_evaluation >= 3 を抽出
            df_remaining = df[~df.index.isin(df_face_sharpness.index)]  # ①で抽出されたデータを除外
            df_overall_evaluation = df_remaining[df_remaining['overall_evaluation'] >= 3]
            self.logger.info(f"Step 2: overall_evaluation >= 3 records: {len(df_overall_evaluation)}")
            
            # 2つのデータセットを結合
            df_combined = pd.concat([df_face_sharpness, df_overall_evaluation], ignore_index=True)
            self.logger.info(f"Total records after combining both conditions: {len(df_combined)}")

            # 結合後にデータが空の場合は処理を終了
            if df_combined.empty:
                self.logger.warning("No records matching the conditions. No output generated.")
                return  

            # 最終データをTXTとして保存
            df_combined.to_csv(self.txt_path, index=False, header=True, sep=',', encoding='utf-8-sig')

            self.logger.info(f"Successfully processed {csv_filename} to {self.txt_filename}")
    ...
    def cleanup(self):
        ...
        if self.txt_path:
            ...
            os.makedirs(target_dir, exist_ok=True)  # コピー先のディレクトリが存在しない場合は作成
            try:
                shutil.copy(self.txt_path, os.path.join(target_dir, self.txt_filename))  # ファイルをコピー

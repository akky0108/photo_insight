...
class PortraitBatchProcessor(BaseBatchProcessor):
                ...
                for row in reader:
                    if row:
                        ...
                        bit_depth = (
                            ...
                            else self.config.get("default_bit_depth", 16)
                        )
    ...
    def _load_image(
        self, file_path: str, orientation: Optional[int], output_bps: Optional[int]
    ) -> Optional[np.ndarray]:
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"CSVファイルや入力フォルダに対応する日付を指定 (例: 2024-08-26). デフォルトは{default_date}です。",
    )

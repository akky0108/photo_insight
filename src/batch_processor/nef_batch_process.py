...
class NEFFileBatchProcess(BaseBatchProcessor):
    ...
    def __init__(self, config_path=None, max_workers=4):
        ...
        self.base_directory_path = Path(self.config.get("base_directory_root", "/mnt/l/picture/2024/"))
    ...
    def get_target_subdirectories(self, base_path: Path, depth: int = 1) -> List[Path]:
        ...
        for subdir in base_path.iterdir():
            if subdir.is_dir():
                ...
                latest_mtime = max((f.stat().st_mtime for f in subdir.glob('**/*') if f.is_file()), default=subdir.stat().st_mtime)
    ...
    def process_directory(self, dir_path: Path) -> None:
        ...
        raw_files = self.exif_handler.read_files(str(dir_path), file_extensions=raw_extensions)
    ...
    def filter_exif_data(self, raw_files: List[ExifData]) -> List[ExifData]:
        ...
        for raw_file in raw_files:
            ...
            filtered_exif = {field: raw_file.get(field, "N/A") for field in self.exif_fields}
            missing_fields = [field for field, value in filtered_exif.items() if value == "N/A"]
            if missing_fields:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, help="設定ファイルパス (デフォルト: './config/config.yaml')")
